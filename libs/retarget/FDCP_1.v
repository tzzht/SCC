...
// \   \  /  \    Timestamp : Mon Aug 19 9:42:47 PDT 2009

//  \___\/\___\
//
// Revision:
//    03/16/09 - Initial version
//    08/19/09 - Second version
...
module FDCP_1 (Q, C, CLR, D, PRE);
    ...
    FDCE #(.INIT(INIT), .IS_C_INVERTED(IS_C_INVERTED), .IS_CLR_INVERTED(IS_CLR_INVERTED)) F1 (.Q(D0), .C(~C), .CE(1'b1), .CLR(CLR), .D(D));
    FDPE #(.INIT(INIT), .IS_C_INVERTED(IS_C_INVERTED), .IS_PRE_INVERTED(IS_PRE_INVERTED)) F2 (.Q(D1), .C(~C), .CE(1'b1), .D(D), .PRE(PRE));
